...
let world=E.createWorld(data,{seed:crypto.getRandomValues(new Uint32Array(1))[0]}),warning='',selected='guzman-01',zoom=1,busy=false,loadError=false,filter='all',selectedCrew='rocio',selectedVendor='mara';
...
window.NeoCourierProfileHost={data,world:()=>world,announce,
 ...
 refresh:()=>render(),
 close(){$('profileLayer').classList.add('hidden');$('crewButton').focus({preventScroll:true});},
...
function render(){
 ...
 $('location').textContent=r?.pending?(busy?'En tránsito a ':'')+data.nodes[r.pending.to].name:node.name;mapRender();$('crewSummary').innerHTML=crewHTML();const count=stock=>Object.values(stock||{}).reduce((n,q)=>n+q,0);$('cargoSummary').textContent=count(r?r.cargo:m.cargo)+' objetos de entrega · '+count(r?r.supplies:totals())+' suministros';
...
$('bootStatus').hidden=!loadError;render();if(loadError)announce(warning);else if(!world.helpSeen)help();else if(!world.run)catalog();window.NeoCourierBack=()=>{if(!$('profileLayer').classList.contains('hidden')){if(profileView)return profileView.back();window.NeoCourierProfileHost.close();return true;}if(!$('battleLayer').classList.contains('hidden'))return combatView?.back()??true;if(itemDialog.open){close(itemDialog);return true;}if(dialog.open){close(dialog);return true;}return false;};
