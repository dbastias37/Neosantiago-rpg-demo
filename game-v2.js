...
function load(){
  try{
    ...
    redistributeOverflow();migrateLegacyInventory();redistributeOverflow();if(typeof narrativeNormalize==="function")narrativeNormalize();return true
  }catch{return false}
}
...
function completeChoice(choice,out,roll,check,extra){
  ...
  state.history.push({day:ev.day,loc:ev.loc,choice:choice.label,result:out.title||choice.title,faction:factionReward});pending={ending:choice.ending||null,returnToRefuge:null,dialogue:typeof narrativeDialogue==="function"?narrativeDialogue(contextualDialogueFor(choice,out)):contextualDialogueFor(choice,out),dialogueSeen:false};
  if(state.morale<=0&&!choice.ending){pending.returnToRefuge="morale";changes.unshift(["Moral","Regreso obligatorio al refugio"])}
...
function continuePendingAdvance(){
  var ending=pending.ending,returnToRefuge=pending.returnToRefuge||(state.morale<=0?"morale":null);pending=null;encounterSaveLocked=false;if(ending){finish(ending);return}
  var oldDay=events[state.index].day;state.index++;if(returnToRefuge){openRefuge(returnToRefuge);return}if(events[state.index].day!==oldDay){night(oldDay)}else{save();render();registerExposedMovement()}
}
...
function enemyPhase(){
  ...
  var continueRound=function(){battleState.enemyActing=null;if(!state.party.some(function(p){return p.hp>0})){loseCombat(false);return}battleState.round++;battleState.acted={};battleState.actor=firstAvailableActor();battleState.busy=false;if(beginActorTurn())renderBattle()};if(lethalFeedback()){settleLethal(continueRound);return}continueRound()
  }
  nextEnemy();
}
...
function eventDisplay(ev,index){var result=legacyEventDisplay(ev,index);result=typeof narrativeEvent==="function"?narrativeEvent(result):result;return typeof narrativeRememberedEvent==="function"?narrativeRememberedEvent(result,index):result}
...
function openPanel(type){
  ...
  $("drawerTitle").textContent=title;$("drawerContent").innerHTML=html;if(type==="decisions"){drawNarrativeMap($("decisionMapPreview"),narrativeMapRoute);$("decisionMapPreview").style.height="auto";$("decisionMapChapter").addEventListener("change",function(){if(routeNarrativeDefs[this.value]){narrativeMapRoute=this.value;openPanel("decisions")}});$("downloadDecisionMap").addEventListener("click",downloadDecisionMap);}Array.prototype.forEach.call(document.querySelectorAll("[data-open-archive]"),function(b){b.addEventListener("click",function(){openArchive(b.dataset.openArchive,b)})});Array.prototype.forEach.call(document.querySelectorAll("[data-open-world-lore]"),function(b){b.addEventListener("click",function(){openWorldLore($("archiveNav"))})});$("shade").classList.remove("hidden");$("drawer").classList.remove("hidden");$("closeDrawer").focus()
}
