...
function refugeReasonData(reason){return{
  ...
  morale:["La moral llegó a cero","Las discusiones quebraron la marcha. Nadie seguirá hacia la superficie hasta reagruparse, atender a los heridos y decidir qué recursos todavía vale la pena conservar."]
...
function continuePendingAdvance(){
  ...
  var oldDay=events[state.index].day;state.index++;if(typeof resolveNarrativeFinale==="function"&&resolveNarrativeFinale())return;if(returnToRefuge){openRefuge(returnToRefuge);return}if(events[state.index].day!==oldDay){night(oldDay)}else{save();render();registerExposedMovement()}
}
...
function night(day){
  state.inhibitor.active=false;state.inhibitor.remainingMs=0;state.inhibitor.exposed=true;state.inhibitor.needsSync=true;state.inhibitor.exposedMoves=0;var hadFood=stockCount("food")>0,hadWater=stockCount("water")>0;if(hadFood){consumeStock("food");addStatItem("itemsUsed","food",1)}if(hadWater)consumeStock("water");var penalty=(hadFood?0:5)+(hadWater?0:8);state.morale=clamp(state.morale-penalty,0,100);state.engineeringUses=3;state.medicalUses=3;state.ordnanceUses=3;
  ...
  $("night").classList.remove("hidden");renderSignalHud();save();render()
}
...
var gameHelpPages=[
  ...
  {title:"Cuidar al grupo",text:"Los HP indican la salud; la energía ayuda a sostener la expedición. Usa medicina para las heridas y alimento para recuperar energía desde las fichas de los personajes.\n\nLa moral pertenece al grupo. Si se agota, tendrán que regresar al refugio. Allí puedes descansar y reagruparte. La comida y el agua también hacen falta al cerrar el día.",tip:"Estar a 0 HP significa quedar agotado. Recupera al personaje antes de volver a contar con él."},
