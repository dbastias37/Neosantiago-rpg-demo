...
function load(){
  try{
    ...
    redistributeOverflow();migrateLegacyInventory();redistributeOverflow();if(typeof narrativeNormalize==="function")narrativeNormalize();if(typeof migrateNarrativeFinale==="function")migrateNarrativeFinale(x);if(typeof fieldNormalize==="function")fieldNormalize();return true
  }catch{return false}
}
...
function confirmLeaveRefuge(){
  if(!state.refuge.active||!refugeCanLeave())return false;closeRefugeHelp(false);state.logisticsSeen=true;state.refuge.active=false;state.refuge.reason=null;state.refuge.message="";$("logisticsModal").classList.add("hidden");$("refuge").classList.add("hidden");save();render();toast("El grupo vuelve a la expedición");setTimeout(function(){openSignalHack("departure")},260);return true
}
