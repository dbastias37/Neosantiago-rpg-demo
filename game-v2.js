...
function openRefuge(reason,restoring){
  ...
  closeRefugeHelp(false);closeGameHelp(false);clearIntroTyping();
...
function signalPauseActive(){
  if(document.hidden||!state.inhibitor||!state.inhibitor.active||state.finished||state.refuge.active||battleState||pending||decisionState||signalModalVisible()||signalWarningVisible())return true;
