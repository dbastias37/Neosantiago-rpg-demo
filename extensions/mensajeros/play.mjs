import {rosaUI} from './rosa-bridge-ui.mjs';
import {medicalUI} from './medical-bridge-ui.mjs';
import {communityNetwork,conversation,placeHeader,contactCard,knownCommunityContacts,completedContact} from './community.mjs?v=5-scarcity';
import {dossier} from './dossier.mjs?v=5-scarcity';
import {animateRoute} from './travel.mjs?v=1';
import * as E from './production.mjs?v=22-community';
...
data.save_key=globalThis.NeoBridgeContext.key(data.save_key);
...
const medical=medicalUI({data,E,getWorld:()=>world,setWorld:w=>{world=w;},canWrite:()=>!loadError,render,open,close:()=>close(dialog),announce,esc,image});
const rosa=rosaUI({data,E,getWorld:()=>world,setWorld:w=>{world=w;},canWrite:()=>!loadError,render,open,close:()=>close(dialog),announce,esc,image});
let bridgeFocus=new URL(location.href).searchParams.get('community');
function showBridge(){
 if(world.run?.pending)return false;
 if(bridgeFocus==='rosa'&&world.rosaBridge){rosa.show();return true;}
 if(world.matiasBridge&&world.matiasBridge.stage!=='delivered'){medical.show();return true;}
 if(world.rosaBridge&&world.rosaBridge.stage!=='delivered'){rosa.show();return true;}
 return false;
}
medical.sync();rosa.sync();
window.addEventListener('message',e=>{if(e.origin===location.origin&&e.source===parent&&e.data?.type==='neo-medical-request'){bridgeFocus=e.data.focus;medical.sync();rosa.sync();render();if(world.helpSeen&&bridgeFocus)showBridge();}});
...
function contact(id){
 ...
 const account=completedContact(data,world,id);
 if(account){
  const local=world.location===c.refuge&&!world.run?.pending;
  open('REGISTRO DE LA COMUNIDAD',conversation({portrait:c.image,name:c.name,place:local?data.nodes[c.refuge].name:'Registro del equipo',title:account.title,body:account.paragraphs.map(p=>'<p>'+esc(p)+'</p>').join(''),note:'Son hechos conservados de las entregas. Consultarlos no inicia otro encargo ni confirma acontecimientos posteriores.',image})+`<div class="dialog-actions"><button data-community-node="${c.refuge}">Volver al refugio</button>${account.receipts.map(id=>'<button data-receipt="'+id+'">Ver entrega: '+esc(data.missions[id].name)+'</button>').join('')}</div>`,'community');return;
 }
 open('CONTACTO DE LA COMUNIDAD',conversation({portrait:c.image,name:c.name,place:world.location===c.refuge&&!world.run?.pending?data.nodes[c.refuge]?.name:'Solicitud conservada por el equipo',title:c.name,body:'<p>«'+esc(m.briefing)+'»</p>',note:world.location===c.refuge?'El equipo se encuentra en este refugio.':'Contacto de la red. Revisa el encargo para conocer el punto de preparación y el traslado necesario.',image})+`<div class="dialog-actions"><button data-community-node="${c.refuge}">Volver al refugio</button><button data-offer="${m.id}" class="primary">Ver encargo</button></div>`,'community');
}
function openJimenez(){
 ...
 open('VICUÑA MACKENNA / OPERACIONES',conversation({portrait:data.missions['jimenez-01'].portrait,name:'Jiménez',place:local?'Vicuña Mackenna':world.jimenezFollowup?.returned?'Registro de la visita':'Informe de Los Leones',title:local?'Lo que sigue sin confirmarse':'Un informe para Jiménez',body:lines.map(line=>'<p>'+esc(line)+'</p>').join(''),note:!local&&world.jimenezFollowup?.returned?'Conversación de la última visita conservada por el equipo.':!local?'Lleva la recepción a Vicuña Mackenna para hablar con Jiménez. El viaje usa tus suministros y puede tener encuentros; no vuelve a pagar el encargo.':'',image})+`<div class="dialog-actions"><button data-community-node="vicuna">Volver al refugio</button><button data-receipt="jimenez-01">Ver entrega</button>${canTravel?'<button data-jimenez-visit class="primary">Viajar a operaciones</button>':''}</div>`,'community');
}
function openGuzman(){
 ...
 open('LOS LEONES / TALLERES',conversation({portrait:data.missions['guzman-01'].portrait,name:'Guzmán',place:local?'Los Leones':world.guzmanFollowup?.returned?'Registro de la visita':'Comprobante de Plaza',title:local?'Lo que encontró la técnica':'Una recepción para Guzmán',body:lines.map(line=>'<p>'+esc(line)+'</p>').join(''),note:!local&&world.guzmanFollowup?.returned?'Conversación de la última visita conservada por el equipo.':!local?'Lleva la recepción a Los Leones para hablar con Guzmán. El viaje usa tus suministros y puede tener encuentros; no vuelve a pagar el encargo.':'',image})+`<div class="dialog-actions"><button data-community-node="leones">Volver al refugio</button><button data-receipt="guzman-01">Ver entrega</button>${canTravel?'<button data-guzman-visit class="primary">Viajar a los talleres</button>':''}</div>`,'community');
}
function openBeatriz(){
 ...
 open('LOS LIBERTADORES / HUERTOS',conversation({portrait:data.missions['beatriz-01'].portrait,name:'Beatriz',place:local?'Los Libertadores':'Registro de la última visita',title:local?'Beatriz y el siguiente turno':'Lo que quedó en los huertos',body:lines.map(line=>'<p>'+esc(line)+'</p>').join(''),note:!local&&world.beatrizFollowup?.returned?'Conversación de la última visita conservada por el equipo.':!local?'Para volver a hablar con Beatriz, el equipo debe viajar a Los Libertadores. Conserva sus heridas y suministros; el recorrido puede tener encuentros. La visita no entrega otro pago.':'',image})+`<div class="dialog-actions"><button data-community-node="libertadores">Volver al refugio</button><button data-receipt="beatriz-01">Ver entrega</button>${canTravel?'<button data-beatriz-visit class="primary">Viajar a los huertos</button>':''}</div>`,'community');
}
...
let gateView=null,gateDismissed=false,seenGate='';
window.NeoCourierGateHost={data,current:()=>E.currentGate(data,world),announce,
 act(action){try{if(loadError)throw Error(warning);world=E.gateAction(data,world,action);save();return true;}catch(error){announce(error.message);return false;}},
 complete(){if(!apply(E.completeGate))return false;gateDismissed=false;if(world.run?.status==='completed'&&E.mission(data,world).type!=='travel')receipt();else if(world.run?.status==='active'&&world.run.pending)pendingModal();return true;},
 close(){gateDismissed=true;$('gateLayer').classList.add('hidden');pendingModal();},
 ready(view){gateView=view;renderGate();}
};
$('gateLayer').src=new URL('./gate.html?v=1',import.meta.url).href;
function renderGate(){
 const gate=E.currentGate(data,world),key=gate?.key||'';
 if(key!==seenGate){seenGate=key;gateDismissed=false;}
 const active=!!gate&&!gateDismissed;$('gateLayer').classList.toggle('hidden',!active);
 if(active){close(dialog);close(itemDialog);gateView?.sync();}
}
...
function render(){
 ...
 $('brief').insertAdjacentHTML('beforeend',returnEntry()+medical.entry()+rosa.entry());
 ...
 $('journal').innerHTML=(r?.log||['La red de encargos espera.']).slice(-10).map(x=>'<p>'+esc(creditText(x))+'</p>').join('');renderBattle();renderGate();
}
...
document.addEventListener('click',ev=>{const b=ev.target.closest('button');if(!b||busy)return;
 if(medical.handle(b)||rosa.handle(b))return;
 if(b.dataset.medicalAction==='return'||b.dataset.rosaAction==='return'){heroes();return;}
 ...
 if(b.dataset.choice){
  ...
  if(apply(E.choose,b.dataset.choice)){
   ...
   if(E.currentGate(data,world))renderGate();
   else if(world.run?.pending?.combat)renderBattle();
 ...
 if(b.id==='helpButton'){help();return;}if(b.hasAttribute('data-help-done')){world.helpSeen=true;save();close(dialog);showBridge();return;}
 if(b.id==='resetCouriers'){open('REINICIAR ENCARGOS',`<div class="dialog-body"><h2 id="dialogTitle">¿Reiniciar?</h2><p>Se borrará el progreso de Los Mensajeros. La expedición conservará los informes que ya recibió y sus ayudas usadas; repetir una entrega no las repone.</p></div><div class="dialog-actions"><button data-close="dialog">Cancelar</button><button data-reset-confirm class="danger">Reiniciar</button></div>`);return;}if(b.hasAttribute('data-reset-confirm')){world=E.createWorld(data,{seed:crypto.getRandomValues(new Uint32Array(1))[0]});loadError=false;selected='relevo-01';zoom=1;save();medical.sync();rosa.sync();render();if(!showBridge())catalog();return;}
 if(['zoomIn','zoomOut','resetMap'].includes(b.id)){zoom=b.id==='resetMap'?1:Math.max(1,Math.min(2.5,zoom+(b.id==='zoomIn'?.25:-.25)));fitMap();}
});
$('bootStatus').hidden=!loadError;render();if(loadError)announce(warning);else if(!world.helpSeen&&!E.currentGate(data,world))help();else if(!world.run&&!showBridge())catalog();window.NeoCourierBack=()=>{if(!$('profileLayer').classList.contains('hidden')){if(profileView)return profileView.back();window.NeoCourierProfileHost.close();return true;}if(!$('battleLayer').classList.contains('hidden'))return combatView?.back()??true;if(!$('gateLayer').classList.contains('hidden'))return gateView?.back()??true;if(!$('refugeLayer').classList.contains('hidden')){if(refugeView)return refugeView.back();window.NeoCourierRefugeHost.close();return true;}if(itemDialog.open){close(itemDialog);return true;}if(dialog.open){close(dialog);return true;}return false;};
