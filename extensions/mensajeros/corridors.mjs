...
export function assignment(data,w,id){
 ...
 return record?.corridorVersion===1&&(data.legacyMissions?.[id]||data.legacyJourneys?.[id])||data.missions[id]||data.journeys?.[id];
}
export function runAssignment(data,w){return w.run?.corridorVersion===1&&(data.legacyMissions?.[w.run.mission]||data.legacyJourneys?.[w.run.mission])||assignment(data,w,w.run?.mission);}
export function scriptAt(data,r,index){return (r.corridorVersion===2?data.corridorScripted:data.scripted)?.[r.mission+':'+index]||null;}
export function prepareCorridors(d){
 ...
 Object.assign(d.missions['romero-01'],{
  ...
  effect:'La reserva queda registrada en República. Los botiquines pasan a costar 6 fichas en Los Héroes; la posta recuerda qué atención quedó pendiente.',
...
export function communityMemory(w,node){
 ...
 const relay=read('relevo-01'),medical=read('romero-01'),survey=read('morales-01'),families=read('ana-01'),lines=[];
