...
function apply(o){
  ...
  (o.add||[]).forEach(function(x){if(!hasPartyItem(x))placePartyItem(x,1)});(o.remove||[]).forEach(function(x){removePartyItem(x,1)});pushUnique(state.docs,o.archive);Object.assign(state.flags,o.flags||{});if(typeof narrativeNormalize==="function")narrativeNormalize();
...
function eventDisplay(ev,index){var result=legacyEventDisplay(ev,index);return typeof narrativeEvent==="function"?narrativeEvent(result):result}
...
function openPanel(type){
  ...
  $("drawerTitle").textContent=title;$("drawerContent").innerHTML=html;if(type==="decisions"){drawNarrativeMap($("decisionMapPreview"),"liraExiledCore");$("downloadDecisionMap").addEventListener("click",downloadDecisionMap);}Array.prototype.forEach.call(document.querySelectorAll("[data-open-archive]"),function(b){b.addEventListener("click",function(){openArchive(b.dataset.openArchive,b)})});Array.prototype.forEach.call(document.querySelectorAll("[data-open-world-lore]"),function(b){b.addEventListener("click",function(){openWorldLore($("archiveNav"))})});$("shade").classList.remove("hidden");$("drawer").classList.remove("hidden");$("closeDrawer").focus()
}
