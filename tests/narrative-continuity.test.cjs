...
for(const copy of [true,false])test('continuous campaign through encounters, conversations and automatic '+(copy?'archive':'testimony'),()=>{
 ...
 for(let step=0;step<500&&!c.state.finished;step++){
  ...
  if(c.pendingNight()){if(c.pendingNight().phase==="planning"){if(c.pendingNight().day===1){c.answerNightCompanion("sara","compartir");c.answerNightCompanion("noa","regreso");c.answerNightCompanion("elias","dudas");}else{assert.ok(c.veraTrade());assert.ok(c.pendingNight().context.includes(c.veraTradeFact(c.veraTrade())));assert.ok(c.evidenceRecord()?.report);assert.match(c.companionConversation("elias",c.pendingNight()).text,/Nodo 14/);c.answerNightCompanion("elias","sostener");assert.ok(c.noaRouteRecord()?.crossing);assert.match(c.companionConversation("noa",c.pendingNight()).text,/Alameda/);c.answerNightCompanion("noa","revisar");assert.ok(c.careRecord()?.decision);assert.match(c.companionConversation("sara",c.pendingNight()).text,/clínica/);c.answerNightCompanion("sara","sostener");}c.settleNight("share");}else c.continueAfterNight();continue}
  ...
  const i=c.state.index===c.veraIndex()&&copy?2:choices.findIndex(o=>!c.reason(o)&&!o.roll);assert.ok(i>=0,'legal deterministic choice at '+c.state.index);c.choose(i);
 }
 ...
 const saved=boot(a.storage).ctx;assert.ok(saved.load());assert.equal(saved.state.ending,c.state.ending);assert.equal(saved.evidenceRecord().verified,true);assert.equal(saved.veraTrade().kind,c.veraTrade().kind);if(saved.veraTrade().kind==="line1")assert.match(saved.state.finaleResolution.story.refuge,/qué entrada conoce Vera/);assert.deepEqual(JSON.parse(JSON.stringify(saved.state.finaleResolution)),JSON.parse(JSON.stringify(c.state.finaleResolution)));
});
