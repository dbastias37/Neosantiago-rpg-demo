...
export function jimenezMemory(w,node){
 ...
 return [r.flags.includes('jimenez_horas_copiadas')?'Jiménez reconoce las dos anotaciones. «Esta es mía. Oí el final y di por hecho que era el aviso de Teresa». Rocío señala que la técnica no pudo confirmar que fuera el mismo. Él corrige su registro: oído parcialmente, sin confirmación.':'Jiménez saca el registro que quedó en Vicuña. Rocío le muestra la casilla pendiente. Él separa la hoja para la próxima comunicación con los talleres. «Tendría que haberles dado esto junto con el módulo».',
 ...
 'Jiménez tacha «esperar confirmación» en la copia de instrucciones y escribe que, si el aviso no recibe respuesta, el relevo debe conservar la última instrucción confirmada y entregar el parte en el puesto. «A Teresa le pedí que esperara algo que quizá no podía llegar». Deja la corrección para el próximo relevo; el equipo no ha visto que Teresa la reciba ni que el enlace quede reparado.'];
}
