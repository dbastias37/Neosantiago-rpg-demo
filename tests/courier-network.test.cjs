...
test('Line 4 visits all eleven real intermediate stations in both directions and keeps Jiménez at Tobalaba',async()=>{
 ...
 let w=E.start(d,E.createWorld(d,{seed:1}),'jimenez-01');w.run.index=11;w.location='colon';w=E.advance(d,w);
...
test('market travel follows existing connections from every station and respects the Baquedano closure',async()=>{
 ...
 for(const node of Object.keys(d.nodes).filter(id=>id!=='heroes')){
  let w=E.createWorld(d,{seed:4});w.location=node;w.credits=30;const before=JSON.stringify(w);w=E.travelHeroes(d,w);const route=E.route(d,w);
...
test('even the final market leg can trigger battle; only victory and finishing loot open trade, with no mission reward',async()=>{
 ...
 for(let seed=1;seed<100;seed++){w=E.createWorld(d,{seed});w.location='republica';w.credits=50;w=E.advance(d,E.travelHeroes(d,w));if(w.run.pending.category==='hostile')break;}
...
test('market encounters persist through reload and retry, and stopping never teleports the group',async()=>{
 const {E,d}=await setup();let w=E.createWorld(d,{seed:4});w.location='leones';w=E.travelHeroes(d,w);const initial=w.location;w=E.advance(d,w);const pending=structuredClone(w.run.pending);w=E.restore(d,E.serialize(w));assert.deepEqual(w.run.pending,pending);
...
test('independent market trips finish, preserve earlier receipts and never award another mission payment',async()=>{
 ...
 for(const origin of ['plaza','libertadores','leones','vicuna']){
  let w=E.createWorld(d,{seed:1});w.location=origin;w.credits=30;w.paid=['ana-01'];w.completed={'ana-01':{amount:30}};w.effects=['ana-01'];
...
test('old pending battles, loot and checkpoint saves migrate without rerolling, resetting HP or charging another leg',async()=>{
 ...
 for(let seed=0;seed<100;seed++){w=E.advance(old,E.start(old,E.createWorld(old,{seed}),'adasme-01'));if(w.run.pending.category==='hostile')break;}
...
test('legacy extraction progress maps repeated stations correctly and returns through the expanded Line 4',async()=>{
 ...
 for(let index=0;index<7;index++){
  let w=E.start(old,E.createWorld(old,{seed:1}),'adasme-01');w.run.index=index;w.location=old.routes['adasme-oriente'].nodes[index];if(index===6)w.run.status='completed';
 ...
 let w=E.advance(old,E.start(old,E.createWorld(old,{seed:1}),'jimenez-01'));w=E.restore(d,E.serialize(w));assert.equal(w.run.pending.id,'jimenez-frecuencia');assert.equal(w.run.index,11);
});
...
test('random market encounters never borrow the fixed scenes from another assignment',async()=>{
 ...
 for(const origin of ['leones','colon','moneda'])for(let seed=1;seed<=30;seed++){
  let w=E.createWorld(d,{seed});w.location=origin;w=E.advance(d,E.travelHeroes(d,w));
...
test('a paid legacy extraction retains its original deadline and penalty after the route expansion',async()=>{
 const {E,d}=await setup(),old=legacy(E);let w=E.start(old,E.createWorld(old,{seed:1}),'adasme-01');
