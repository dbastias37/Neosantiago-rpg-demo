...
function load(){
  try{
    ...
    redistributeOverflow();migrateLegacyInventory();redistributeOverflow();if(typeof narrativeNormalize==="function")narrativeNormalize();if(typeof migrateNarrativeFinale==="function")migrateNarrativeFinale(x);return true
  }catch{return false}
}
...
function signalPauseActive(){
  if(itemDetailState||document.hidden||!state.inhibitor||!state.inhibitor.active||state.finished||state.refuge.active||battleState||pending||decisionState||signalModalVisible()||signalWarningVisible())return true;
...
function render(){
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-choice]"),function(b){b.addEventListener("click",function(){choose(Number(b.dataset.choice))})});renderSignalHud();renderMini()
}
function renderMini(){
