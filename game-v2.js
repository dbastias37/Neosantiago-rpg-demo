...
function fresh(){return{version:3,campaignRevision:3,inventoryRevision:4,progressionRevision:1,introCompleted:false,index:0,threat:18,morale:64,credits:0,factionPoints:0,tradeStock:tradeStockForDay(1),tradeStockDay:1,armorerStock:armorerStockForDay(1),armorerStockDay:1,starterKitGiven:false,refuge:{active:false,reason:null,rested:false,rejoined:false,visits:0,npc:"mara",message:""},summarySeen:false,finalTier:null,score:0,engineeringUses:2,medicalUses:2,ordnanceUses:2,res:{food:0,water:4,meds:0,ammo:0,battery:0},cons:{bandage:0,emp:0,grenade:0},party:[
...
var state=fresh(),pending=null,battleState=null,toastTimer=null,lootInterval=null,archiveOpener=null,worldLoreOpener=null,introStep=0,profileTab="inventory";
...
function professionWorkshopHtml(p){
  ...
  var sara=p.id==="sara",noa=p.id==="noa",title=sara?"Mesa médica de Sara":noa?"Banco de explosivos de Noa":"Taller de Elías",description=sara?"Preparar medicina consume una acción médica. La tela se recupera de merodeadores.":noa?"Fabrica granadas y cargas EMP con chatarra electrónica recuperada.":"Reparar o fabricar consume una acción técnica. El uso de armas de fuego tiene −2 de precisión.",meta=sara?state.medicalUses+" / 2 acciones · "+stockCount("cloth")+" tela · "+stockCount("water")+" agua":noa?state.ordnanceUses+" / 2 acciones · "+stockCount("electronics")+" electrónica · "+stockCount("pulseCore")+" núcleos":state.engineeringUses+" / 2 acciones · "+stockCount("scrap")+" componentes · "+(hasPartyItem("tool")?"herramientas listas":"sin herramientas");
...
function renderProfile(i,feedback){
  ...
  var bag=p.bag.length?p.bag.map(function(entry,index){var d=gear(entry.id),equip=canEquip(p,d),usable=profileItemUsable(p,entry.id),label=d&&d.kind==="weapon"&&p.categories.indexOf(d.category)<0?"Categoría incompatible":"Equipar",targets=state.party.map(function(other,j){return j!==i&&canReceive(other,entry.id)?'<option value="'+j+'">'+esc(other.name)+' · '+bagUsed(other)+'/'+bagCapacity(other)+'</option>':""}).join(""),dur=d&&d.maxDurability?' · Durabilidad '+entry.durability+'/'+d.maxDurability:"",actions=(d&&d.slot?'<button class="equip-btn" data-equip="'+index+'" '+(!equip?'disabled':'')+'>'+esc(label)+'</button>':"")+(usable||["food","meds","bandage","medkit"].indexOf(entry.id)>=0?'<button class="profile-use-btn" data-profile-use="'+index+'" '+(!usable?'disabled':'')+'>Usar</button>':"");return'<article class="bag-item">'+itemArt(entry.id,d&&d.name)+'<span class="bag-copy"><b>'+esc((d?d.name:entry.id)+(entry.qty>1?" ×"+entry.qty:""))+'</b><small>'+esc((d?d.desc:"Objeto recuperado")+dur)+'</small></span><div class="bag-actions">'+actions+'<select class="transfer-select" data-transfer="'+index+'" '+(!targets?'disabled':'')+' aria-label="Transferir '+esc(d?d.name:entry.id)+'"><option value="">Transferir a…</option>'+targets+'</select></div></article>'}).join(""):'<p class="empty">La mochila está vacía.</p>';
...
function closeProfile(){$('profileModal').classList.add('hidden');if(state.refuge.active)renderRefuge()}
...
function transferBagItem(fromIndex,bagIndex,toIndex){
  if(battleState)return;var from=state.party[fromIndex],to=state.party[toIndex],entry=from&&from.bag[bagIndex],d=entry&&gear(entry.id);if(!from||!to||!entry||!canReceive(to,entry.id)){toast("El destinatario no tiene espacio");renderProfile(fromIndex);return}if(!addToBag(to,entry.id,1,entry.durability)){toast("No se pudo transferir el objeto");return}entry.qty--;if(entry.qty<=0)from.bag.splice(bagIndex,1);save();renderMini();renderProfile(fromIndex);if(state.refuge.active)renderRefuge();toast((d?d.name:entry.id)+" transferido a "+to.name)
}
...
function night(day){
  var hadFood=stockCount("food")>0,hadWater=stockCount("water")>0;if(hadFood){consumeStock("food");addStatItem("itemsUsed","food",1)}if(hadWater)consumeStock("water");var penalty=(hadFood?0:5)+(hadWater?0:8);state.morale=clamp(state.morale-penalty,0,100);state.engineeringUses=2;state.medicalUses=2;state.ordnanceUses=2;
  ...
  $("nightChanges").innerHTML=[["Alimento común",hadFood?"−1":"Agotado"],["Energía",hadFood?"+24":"−8"],["Recuperación","+8 HP"],["Talleres profesionales","2 acciones"]].map(function(x){return'<div class="result-chip">'+x[0]+"<strong>"+x[1]+"</strong></div>"}).join("");
...
function generateLoot(enemy){
  var group=enemy.lootGroup||enemy.type,out=[];if(group==="merodeador"){addLootDrop(out,"cloth",rand(1,2));addLootDrop(out,"scrap",rand(1,2));if(random()<.32)addLootDrop(out,"ammo9",rand(1,3));if(random()<.22)addLootDrop(out,"food",1);if(random()<.12)addLootDrop(out,"bandage",1);if(random()<.05)addLootDrop(out,"revolver",1);if(random()<.015){addLootDrop(out,"shotgun12",1);addLootDrop(out,"shell12",1)}if(random()<.02)addLootDrop(out,"grenade",1)}
  ...
  else{addLootDrop(out,"scrap",rand(1,2));addLootDrop(out,"electronics",1);if(random()<.18)addLootDrop(out,"pulseCore",1);if(random()<.30)addLootDrop(out,"ammo556",rand(1,3));if(random()<.15)addLootDrop(out,random()<.25?"meds":"bandage",1);if(random()<.05)addLootDrop(out,random()<.55?"pistol9":"rifle556",1);if(random()<.04)addLootDrop(out,random()<.5?"helmetRiot":"vestTactical",1);if(random()<.02)addLootDrop(out,"grenade",1)}return out
}
...
document.addEventListener("keydown",function(e){if(!$("titleScreen").classList.contains("hidden")&&(e.key==="Enter"||e.key===" ")){e.preventDefault();enterTitleScreen();return}if(e.key==="Escape"&&!$("worldLoreModal").classList.contains("hidden")){closeWorldLore();return}if(e.key==="Escape"&&!$("archiveModal").classList.contains("hidden")){closeArchive();return}if(e.key==="Escape"&&!battleState){closePanel();closeProfile()}if(battleState||!$("titleScreen").classList.contains("hidden")||!$("start").classList.contains("hidden")||!$("gameIntro").classList.contains("hidden")||!$("worldLoreModal").classList.contains("hidden")||!$("result").classList.contains("hidden")||!$("night").classList.contains("hidden")||!$("final").classList.contains("hidden")||!$("summary").classList.contains("hidden")||!$("refuge").classList.contains("hidden")||!$("profileModal").classList.contains("hidden")||!$("archiveModal").classList.contains("hidden"))return;if(["1","2","3"].indexOf(e.key)>=0)choose(Number(e.key)-1)});
