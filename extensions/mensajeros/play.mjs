...
function catalog(){
 ...
 const sample=data.missions['guzman-01'];open('CENTRAL DE ENCARGOS',`<div class="dialog-body"><h2 id="dialogTitle" tabindex="-1">Trabajo entre refugios</h2>${progressHTML()}<p>Después del plazo se descuentan ${sample.late_penalty} fichas por cada ${sample.late_step_minutes} minutos adicionales, hasta la mitad del pago.</p><div class="catalog-tabs">${[['all','Conocidos'],['available','Disponibles'],['active','En curso'],['done','Entregados']].map(([id,label])=>`<button data-filter="${id}" aria-pressed="${filter===id}">${label}</button>`).join('')}</div><div class="offer-grid">${list.map(m=>`<article class="offer">${image(m.portrait,m.issuer)}<div class="eyebrow">${esc(m.issuer)} / ${world.paid.includes(m.id)?'ENTREGADO':(E.departurePlan(data,world,m.id)?.legs??data.routes[m.route].edges.length)+' TRAMOS'}</div><h3>${esc(m.name)}</h3><p>${esc(m.summary)}</p><small>Plazo ${E.departurePlan(data,world,m.id)?.limit??m.time_limit} min · ${m.reward.base}${m.reward.stealth_bonus?' + '+m.reward.stealth_bonus+' por sigilo':''} fichas</small><button data-offer="${m.id}">${world.paid.includes(m.id)?'Ver resultado':'Hablar con '+m.issuer}</button></article>`).join('')||'<p>No hay encargos en esta categoría.</p>'}</div></div>`);
}
function offer(id){
 if(!E.missionOpen(data,world,id)){announce('Este contacto aún no ha abierto su encargo.');return;}
 selected=id;const m=data.missions[id],complete=world.paid.includes(id),active=world.run&&['active','failed'].includes(world.run.status),plan=E.departurePlan(data,world,id);
 if(!world.run)render();
 open('ENCARGO / '+m.issuer,`<div class="dialog-body"><div class="hero-brief">${image(m.portrait,m.issuer)}<div><h2 id="dialogTitle" tabindex="-1">${esc(m.name)}</h2><p class="contact-introduction">${esc(m.introduction)}</p><p>«${esc(m.briefing)}»</p><p class="muted">Destino: ${esc(m.recipient)} · Plazo al aceptar: ${plan?.limit??m.time_limit} minutos · Recompensa: ${m.reward.base}${m.reward.stealth_bonus?' + '+m.reward.stealth_bonus+' sin combates':''} fichas.</p><p>Después del plazo, cada ${m.late_step_minutes} minutos restan ${m.late_penalty} fichas. Combatir, descansar, desviarse y saquear consumen tiempo; leer o cerrar el juego no.</p></div></div><div class="outcome"><div class="eyebrow">${m.pickup?'CARGA QUE DEBES RECOGER':'CARGA DEL ENCARGO'}</div>${itemRows({...m.cargo,...m.issued})}</div><p>${esc(m.effect)}</p>${!complete&&plan?'<p class="muted">Preparación: '+esc(data.nodes[plan.destination].name)+'. '+plan.legs+' tramos de encargo desde allí. '+(plan.journey?'Primero debes llegar; el traslado tiene su propio tiempo y no descuenta el pago.':'El equipo ya está aquí. Puedes recibir el encargo cuando esté preparado.')+'</p>':''}</div><div class="dialog-actions"><button data-catalog>Volver</button><button data-preview-route="${m.id}">Revisar ruta</button>${complete?'<button data-receipt="'+id+'" class="primary">Ver resultado</button>':active?'<button data-resume class="primary">Retomar encargo actual</button>':!E.atMissionOrigin(data,world,id)?'<button data-approach="'+id+'" class="primary">Revisar traslado</button>':'<button data-accept="'+id+'" class="primary">Aceptar encargo</button>'}</div>`);
}
...
function displayRoute(preview){
 ...
 const m=preview?data.missions[preview]:world.run?E.mission(data,world):data.missions[selected],r=data.routes[m.route];
 ...
 open('ITINERARIO / '+m.issuer,`<div class="dialog-body"><h2 id="dialogTitle" tabindex="-1">${esc(m.name)}</h2><p>Tiempo base: ${r.edges.slice(startIndex).reduce((n,e)=>n+e.minutes,0)} min.${m.type==='travel'?' Puede haber encuentros en cada tramo.'+(m.purpose==='assignment'?' El encargo todavía no ha comenzado.':' El mercado abre al llegar a Los Héroes.'):' Plazo con margen: '+limit+' min.'}</p>${itineraryHTML(r,startIndex,current)}</div><div class="dialog-actions"><button data-close="dialog" class="primary">Volver</button></div>`);
}
function approach(id){
 ...
 open('ANTES DEL ENCARGO',`<div class="dialog-body"><div class="hero-brief">${image(m.portrait,m.issuer)}<div><h2 id="dialogTitle" tabindex="-1">Llegar a ${esc(data.nodes[plan.destination].name)}</h2><p>${esc(m.issuer)} recibió tu aviso. La carga y el equipo del encargo se entregan al llegar; por ahora viajas con tus mochilas.</p><p>Sales de ${esc(data.nodes[world.location].name)}. ${rt.edges.length} tramos · ${rt.edges.reduce((n,e)=>n+e.minutes,0)} minutos de desplazamiento base.</p></div></div><p>Puede haber encuentros, combate y desgaste. El plazo de ${plan.limit} minutos del encargo empieza solo cuando lo aceptes en destino.</p><p class="time-warning">Llevan ${stock.food||0} raciones, ${stock.water||0} aguas y ${stock.medkit||0} botiquines. Revisa sus heridas y el equipo antes de salir.</p>${itineraryHTML(rt)}</div><div class="dialog-actions"><button data-offer="${id}">Volver</button><button id="cargoCrewButton">Revisar equipo</button>${E.atHeroes(data,world)?'<button data-heroes>Mara / Armero</button>':''}<button data-start-approach="${id}" class="primary">Viajar al encuentro</button></div>`);
}
