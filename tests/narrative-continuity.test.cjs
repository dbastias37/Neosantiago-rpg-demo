...
for(const copy of [true,false])test('continuous campaign through encounters, conversations and automatic '+(copy?'archive':'testimony'),()=>{
 ...
 for(let step=0;step<500&&!c.state.finished;step++){
  ...
  if(c.pendingNight()){if(c.pendingNight().phase==="planning"){if(c.pendingNight().day===1){c.answerNightCompanion("sara","compartir");c.answerNightCompanion("noa","regreso");}else{assert.ok(c.noaRouteRecord()?.crossing);assert.match(c.companionConversation("noa",c.pendingNight()).text,/Alameda/);c.answerNightCompanion("noa","revisar");assert.ok(c.careRecord()?.decision);assert.match(c.companionConversation("sara",c.pendingNight()).text,/clínica/);c.answerNightCompanion("sara","sostener");}c.settleNight("share");}else c.continueAfterNight();continue}
