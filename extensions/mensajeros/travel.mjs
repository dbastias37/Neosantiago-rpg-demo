...
export function animateRoute({path,marker,destination,reducedMotion=false,brisk=false,isCurrent=()=>true,
 requestFrame=callback=>requestAnimationFrame(callback),now=()=>performance.now()}){
 const length=path.getTotalLength(),duration=reducedMotion?0:brisk?350:Math.max(900,Math.min(2600,length*3));
 ...
 return new Promise(resolve=>{
  function frame(timestamp){
   ...
   if(arrived!==null&&timestamp-arrived>=(brisk?150:1000)){marker.classList.remove('arriving');resolve(true);return;}
   requestFrame(frame);
  }
  requestFrame(frame);
 });
