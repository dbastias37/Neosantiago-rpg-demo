...
function showGameIntro(step){
  clearIntroTyping();introStep=clamp(Math.floor(Number(step)||0),0,introPages.length-1);state.introPage=introStep;
  var page=introPages[introStep];$("introChapter").textContent=page.chapter;$("introTitle").textContent=page.title;
  $("introImage").src=assetUrl(page.image);$("introImage").alt=page.alt;$("introLocation").textContent=page.location;
  ...
  introTypeTimer=setInterval(function(){
    ...
    if(document.hidden||!$("worldLoreModal").classList.contains("hidden"))return;
...
function introKeydown(e){
  ...
  if(e.key==="Tab"){
    var buttons=[introTypeTimer?$("introReveal"):null,!introTypeTimer&&introStep>0?$("introBack"):null,!introTypeTimer?$("introNext"):null,$("openWorldLore")].filter(Boolean);
