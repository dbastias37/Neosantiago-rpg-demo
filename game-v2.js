...
function showRouteNarrativeOptions(){
  ...
  routeNarrativeState.optionsVisible=true;renderRouteNarrativeChoices();$("routeNarrativeChoices").classList.remove("hidden");$("routeNarrativeClock").classList.remove("hidden");$("routeNarrativeButton").textContent="Elegir ruta";$("routeNarrativeButton").disabled=true;clearInterval(routeNarrativeTimer);routeNarrativeTimer=setInterval(function(){var elapsed=Date.now()-start,pct=clamp(100-Math.round(100*elapsed/duration),0,100);$("routeNarrativeTimeBar").style.width=pct+"%";if(elapsed>=duration){clearInterval(routeNarrativeTimer);routeNarrativeTimer=null;selectRouteNarrativeChoice(routeDefaultChoice(scene,def))}},100);var first=$("routeNarrativeChoices").querySelector("button:not(:disabled)");if(first)first.focus()
}
function revealRouteNarrativeText(){
  if(!routeNarrativeState)return;clearInterval(routeNarrativeTypeTimer);routeNarrativeTypeTimer=null;$("routeNarrativeText").textContent=routeNarrativeState.text;if(routeNarrativeState.lineIndex>=routeNarrativeState.lines.length-1)showRouteNarrativeOptions();else $("routeNarrativeButton").focus()
}
