...
function signalPauseActive(){
  if(document.hidden||!state.inhibitor||!state.inhibitor.active||state.finished||state.refuge.active||battleState||pending||signalModalVisible()||signalWarningVisible())return true;
...
function choose(i){
  if(pending||battleState)return;var ev=eventDisplay(events[state.index],state.index),c=ev.choices[i];if(!c||reason(c))return;
