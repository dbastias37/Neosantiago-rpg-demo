...
export function start(data,source,id){
 ...
 r.startIndex=entry.index;r.index=entry.index;
...
function arrive(data,w){const r=w.run,p=r.pending;r.history.push({id:p.id,category:p.category,node:p.to});r.index=p.edgeIndex+1;r.pending=null;w.location=Base.here(data,w).id;r.log.push('Llegada a '+Base.here(data,w).name+'.');updateCheckpoint(data,w);}
