...
for(const copy of [true,false])test('continuous campaign through encounters, conversations and automatic '+(copy?'archive':'testimony'),()=>{
 ...
 const saved=boot(a.storage).ctx;assert.ok(saved.load());assert.equal(saved.state.ending,c.state.ending);assert.equal(saved.evidenceRecord().verified,true);assert.deepEqual(JSON.parse(JSON.stringify(saved.state.finaleResolution)),JSON.parse(JSON.stringify(c.state.finaleResolution)));
});
