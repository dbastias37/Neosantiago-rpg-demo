...
test('the main story introduces Varela before Mara and starts only after the player continues',()=>{const a=boot(),c=a.ctx;assert.equal(c.introPages.some(p=>/Mara/.test(p.chapter+p.title+p.text)),false);intro(c);assert.equal(c.state.activity,'hub');assert.equal(c.state.refuge.active,false);assert.equal(c.state.starterKitGiven,false);assert.equal(c.activityVisible(),true);c.resumeStoryActivity();assert.equal(a.nodes.get('storyPrelude').classList.contains('hidden'),false);assert.equal(c.state.refuge.active,false);assert.equal(c.state.storyPreludeSeen,false);c.finishStoryPrelude();assert.equal(c.state.storyPreludeSeen,true);assert.equal(c.state.activity,'story');assert.equal(c.state.refuge.active,true);assert.equal(c.state.refuge.visits,1);});
