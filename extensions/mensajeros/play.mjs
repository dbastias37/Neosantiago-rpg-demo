...
import * as E from './production.mjs?v=21-compuertas';
...
medical.sync();
window.addEventListener('message',e=>{if(e.origin===location.origin&&e.source===parent&&e.data?.type==='neo-medical-request'){medical.sync();render();}});
...
function render(){
 ...
 $('brief').insertAdjacentHTML('beforeend',returnEntry()+medical.entry());
...
document.addEventListener('click',ev=>{const b=ev.target.closest('button');if(!b||busy)return;
 if(medical.handle(b))return;
 if(b.dataset.medicalAction==='return'){heroes();return;}
