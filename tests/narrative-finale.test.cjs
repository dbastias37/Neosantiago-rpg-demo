...
test('Vera agreement is prepared earlier and physical relay realignment cancels it',()=>{
 const c=session().ctx;c.state.index=17;c.choose(3);assert.ok(c.state.flags.veraExtractionReserved);assert.equal(c.pending.dialogue,null);c.advance();c.settleNight("share");c.continueAfterNight();c.state.index=20;c.placePartyItem('battery',1);c.choose(3);assert.equal(c.state.flags.relayPrepared,true);assert.equal(c.state.flags.veraExtractionReserved,false);
});
