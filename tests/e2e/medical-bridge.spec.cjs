...
for(const variant of ['A','B'])test(`medical bridge ${variant}: request, physical receipt, reload and refuge memory`,async({page},testInfo)=>{
 const {E,d}=await setup(),key='neosantiago.lab.bridge.'+variant+'.'+d.save_key;
 ...
 expect(await page.evaluate(()=>[localStorage.getItem('neosantiago2130_demo_v3'),localStorage.getItem('neosantiago.mensajeros.production.v1')])).toEqual(['production sentinel','courier sentinel']);
});
