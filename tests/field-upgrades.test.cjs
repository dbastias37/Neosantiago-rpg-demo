...
test('four victory rewards have no duplicates, persist across reload and do not consume narrative RNG',()=>{
...
test('story progress alone grants nothing; first actual victory unlocks reward once',()=>{
 const a=core(),c=a.ctx;c.state.index=21;assert.equal(c.fieldDue(),false);assert.equal(c.fieldStore().selected.length,0);const b=c.battleState;c.battleState.enemies.forEach(e=>e.hp=0);c.beginLootPhase();c.finishLooting();assert.equal(c.fieldStore().victories,1);assert.equal(c.fieldDue(),true);c.fieldRecordVictory(b);assert.equal(c.fieldStore().victories,1);
 const d=core().ctx;d.loseCombat(true);assert.equal(d.fieldStore().victories,0);assert.equal(d.fieldDue(),false);
});
test('new runs hide upgrades, teach earned reward, explain passive use and show two-win progress',()=>{
 const a=ui(),c=a.ctx,d=a.document;c.fieldScheduleOffer();assert.ok(d.getElementById('fieldCollection').classList.contains('hidden'));c.state.index=20;assert.equal(c.fieldMaybeOffer(),false);
 c.fieldRecordVictory({});c.fieldMaybeOffer();assert.match(d.getElementById('fieldCards').textContent,/Las pasivas funcionan solas/);assert.equal(c.fieldBack(),false);assert.equal(c.fieldStore().tutorialSeen,false);
 a.click(d.querySelector('[data-field-reward-next]'));assert.equal(c.fieldStore().tutorialSeen,true);c.fieldStore().offer={tier:0,ids:['plate','arc','shared']};c.fieldOpen('offer');const select=d.querySelector('[data-field-owner="plate"]');Object.defineProperty(select,'value',{value:'sara'});a.click(d.querySelector('[data-field-choose="plate"]'));assert.match(d.getElementById('fieldCards').textContent,/automáticamente/);assert.match(d.getElementById('fieldInstruction').textContent,/0\/2 victorias/);a.click(d.querySelector('[data-field-equipped-done]'));
 c.fieldRecordVictory({});assert.equal(c.fieldDue(),false);assert.match(c.fieldProgressText(),/1\/2 victorias/);c.fieldRecordVictory({});assert.equal(c.fieldDue(),true);c.fieldMaybeOffer();assert.equal(d.querySelector('.field-tutorial-steps'),null);
});
test('migration preserves equipped cards without awarding old wins or story progress',()=>{
 const c=boot().ctx;c.state.index=22;c.state.stats.wins=15;c.state.fieldUpgrades={version:1,selected:[],offer:{tier:0,ids:['plate','arc','shared']}};c.fieldNormalize();assert.equal(c.fieldDue(),false);assert.equal(c.fieldStore().offer,null);
 c.state.fieldUpgrades={version:1,selected:[{id:'plate',owner:'sara'}]};c.fieldNormalize();assert.equal(c.fieldStore().selected.length,1);assert.equal(c.fieldDue(),false);c.fieldRecordVictory({});assert.equal(c.fieldDue(),false);c.fieldRecordVictory({});assert.equal(c.fieldDue(),true);
});
