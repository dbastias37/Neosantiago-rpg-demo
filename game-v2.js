...
function beginLoot(i){
  if(!battleState||battleState.phase!=="loot"||battleState.busy)return;if(battleState.looter===null){toast("Primero selecciona quién saquea");return}var currentBattle=battleState,e=currentBattle.enemies[i];if(!e||e.looted||e.searching)return;
...
function continueGame(){closeItemDetails(false);closeRefugeHelp(false);closeGameHelp(false);clearDecision();if(!load()){newGame();return}gameSessionActive=true;encounterSaveLocked=false;signalLastTick=Date.now();$("titleScreen").classList.add("hidden");$("start").classList.add("hidden");render();if(!state.introCompleted)showGameIntro(state.introPage);else if(state.finished&&state.ending){if(state.summarySeen)showRunSummary();else finish(state.ending)}else if(state.refuge.active)openRefuge(state.refuge.reason,true);else if(state.inhibitor.pendingContact==="hack"||state.inhibitor.pendingContact==="exposed")startTrackingCombat(state.inhibitor.pendingContact);else{toast("Partida restaurada");if(state.inhibitor.needsSync&&!state.inhibitor.active)setTimeout(function(){openSignalHack("restore")},320)}}
