...
function displayRoute(preview){const m=preview?data.missions[preview]:world.run?E.mission(data,world):data.missions[selected],r=data.routes[m.route];open('ITINERARIO / '+m.issuer,`<div class="dialog-body"><h2 id="dialogTitle" tabindex="-1">${esc(m.name)}</h2><p>Tiempo base: ${r.edges.reduce((n,e)=>n+e.minutes,0)} min.${m.type==='travel'?' Puede haber encuentros en cada tramo.':' Plazo con margen: '+m.time_limit+' min.'}</p><ol class="route-list">${r.nodes.map((id,i)=>{const n=data.nodes[id],edge=r.edges[i];return `<li class="${world.run&&i===world.run.index?'active':''}"><span class="number">${String(i+1).padStart(2,'0')}</span><span>${esc(n.name)}</span><small>${edge?edge.minutes+' min al próximo punto':'Destino'}</small></li>`;}).join('')}</ol></div><div class="dialog-actions"><button data-close="dialog" class="primary">Volver</button></div>`);}
...
function restModal(){const f=E.rewardForecast(data,world);open('PUNTO DE DESCANSO',`<div class="dialog-body"><h2 id="dialogTitle" tabindex="-1">Recuperar al grupo</h2><p>Consume 1 ración y 1 agua, recupera resistencia y 12 HP por persona. Avanza 10 minutos.</p><p class="time-warning">Pago estimado: ${f?f.amount+' fichas · plazo '+f.limit+' min.':'Sin recompensa por viaje. El descanso añade 10 minutos.'}</p></div><div class="dialog-actions"><button data-close="dialog">Seguir</button><button data-rest-confirm class="primary">Descansar</button></div>`);}
...
function mapRender(){
 ...
 [...new Set([...rt.nodes,...data.contacts.map(c=>c.refuge)])].filter(id=>data.nodes[id]).forEach(id=>{const n=data.nodes[id],el=svgEl(n.checkpoint?'rect':'circle',n.checkpoint?{x:n.x-7,y:n.y-7,width:14,height:14}:{cx:n.x,cy:n.y,r:5});el.setAttribute('class',n.checkpoint?'control-point':'instance-point');el.setAttribute('role','button');el.setAttribute('aria-label',n.name);const title=svgEl('title',{});title.textContent=n.name;el.append(title);el.setAttribute('tabindex','0');el.addEventListener('click',()=>nodeInfo(id));el.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();nodeInfo(id);}});layer.append(el);if(n.line==='L4'){const left=n.x>=977,label=svgEl('text',{x:n.x+(left?-12:12),y:n.y+4,class:'route-station-label back','text-anchor':left?'end':'start'});label.textContent=n.name;layer.append(label);}});
...
function render(){
 ...
 $('brief').innerHTML=`<div class="mission-portrait">${image(m.portrait,m.issuer)}<span class="portrait-name">${esc(m.issuer)}</span></div><span class="chip">${r?({active:'EN CURSO',completed:m.type==='travel'?'EN LOS HÉROES':'ENTREGADO',failed:'VIAJE INTERRUMPIDO',abandoned:m.type==='travel'?'VIAJE DETENIDO':'DEVUELTO'})[r.status]:'DISPONIBLE'}</span><h2>${esc(m.name)}</h2><p>${esc(m.summary)}</p>${m.type==='travel'?'<button class="primary" data-journey-route>Ver recorrido</button>':'<button class="primary" data-offer="'+m.id+'">Ver encargo</button>'}${r&&['active','failed'].includes(r.status)?'<button class="text-button" data-abandon>'+(m.type==='travel'?'Detener viaje':'Devolver encargo')+'</button>':''}`;
 $('location').textContent=r?.pending?(busy?'En tránsito a ':'')+data.nodes[r.pending.to].name:node.name;mapRender();$('crewSummary').innerHTML=crewHTML();const count=stock=>Object.values(stock||{}).reduce((n,q)=>n+q,0);$('cargoSummary').textContent=count(r?r.cargo:m.cargo)+' objetos de entrega · '+count(r?.status==='active'?r.supplies:totals())+' suministros';
