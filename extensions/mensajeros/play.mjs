import {dossier} from './dossier.mjs?v=1';
...
function open(kicker,html,view=''){if(busy)return;dialog.classList.toggle('assignment-dossier',view==='dossier');$('dialogKicker').textContent=kicker;$('dialogBody').innerHTML=html;if(!dialog.open)dialog.showModal();dialog.scrollTop=0;$('dialogBody').scrollTop=0;$('dialogTitle')?.focus({preventScroll:true});}
...
function openBeatriz(){
 ...
 open('LOS LIBERTADORES / HUERTOS',`<div class="dialog-body"><div class="hero-brief mission-detail-header">${image(data.missions['beatriz-01'].portrait,'Beatriz')}<div><h2 id="dialogTitle" tabindex="-1">${local?'Beatriz y el siguiente turno':'Lo que quedó en los huertos'}</h2><p>${local?'Los Libertadores':'Registro de la última visita'}</p></div></div>${lines.map(line=>'<p>'+esc(line)+'</p>').join('')}${!local?'<p class="muted">Para volver a hablar con Beatriz, el equipo debe viajar a Los Libertadores. Conserva sus heridas y suministros; el recorrido puede tener encuentros. La visita no entrega otro pago.</p>':''}</div><div class="dialog-actions"><button data-close="dialog">Volver</button><button data-receipt="beatriz-01">Ver entrega</button>${canTravel?'<button data-beatriz-visit class="primary">Viajar a los huertos</button>':''}</div>`);
}
...
document.addEventListener('click',ev=>{const b=ev.target.closest('button');if(!b||busy)return;
 ...
 if(b.dataset.previewRoute){displayRoute(b.dataset.previewRoute);return;}if(b.dataset.contact){contact(b.dataset.contact);return;}if(b.id==='contactsButton'){contacts();return;}
