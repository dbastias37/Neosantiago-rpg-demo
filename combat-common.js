...
function enemyUnitHtml(e,i,lootPhase){
  ...
  if(lootPhase){
    var pending=e.looted&&e.loot.some(function(x){return x.status==="pending"}),available=!e.searching&&(!e.looted||pending),stateClass=pending?"lootable":e.looted?"looted":e.searching?"":"lootable";
...
function beginLoot(i){
  if(!battleState||battleState.phase!=="loot"||battleState.busy)return;if(battleState.looter===null){toast("Primero selecciona quién saquea");return}var currentBattle=battleState,e=currentBattle.enemies[i];if(!e||e.searching)return;if(e.looted){if(e.loot.some(function(x){return x.status==="pending"}))renderLootModal(i);return;}
...
function pendingLootUnits(loot){return(loot||[]).reduce(function(sum,drop){return sum+(drop.status==="pending"&&!drop.collectible?Math.max(1,Number(drop.qty)||1):0)},0)}

function canTakeAllLoot(p,loot){
  var pending=pendingLootUnits(loot);return !!(p&&(loot||[]).some(function(x){return x.status==="pending"})&&pending<=bagFree(p))
}

function renderLootModal(enemyIndex){
  if(!battleState)return;var e=battleState.enemies[enemyIndex],p=state.party[battleState.looter],used=bagUsed(p),capacity=bagCapacity(p),free=bagFree(p),percent=Math.round(100*used/capacity),pending=pendingLootUnits(e.loot),collectibles=e.loot.filter(function(x){return x.collectible&&x.status==="pending"}).length;battleState.openLoot=enemyIndex;$("lootTitle").textContent=e.name+" · loot";$("lootOwner").textContent=p.name+" registra el cuerpo";$("lootCapacityText").textContent=used+" / "+capacity+" espacios ocupados";$("lootCapacityPercent").textContent=percent+"%";$("lootCapacityBar").style.width=percent+"%";$("lootCapacity").classList.toggle("full",free===0);$("lootGuidance").textContent=pending>free?"No cabe todo el loot. La recolección no ocupa mochila; puedes tomarla igualmente.":"Selecciona los objetos que quieres llevar o recoge todo el loot disponible.";$("lootMessage").textContent=battleState.lootMessage||"";$("lootMessage").classList.toggle("hidden",!battleState.lootMessage);
  $("lootItems").innerHTML=e.loot.map(function(drop,i){var d=drop.collectible?battleCollectibles[drop.id]:gear(drop.id),done=drop.status!=="pending",units=Math.max(1,Number(drop.qty)||1),dis=drop.collectible?'':lootDisassembleAction(i,drop),status=drop.collectible?"Recolección · no ocupa mochila":drop.status==="disassembled"?"Desarmado por Elías":drop.status==="broken"?"Elemento roto":"Ocupa "+units+" espacio"+(units===1?'':'s'),art=drop.collectible?'<span class="collection-loot-mark">'+collectibleGlyph(drop.id)+'</span>':itemArt(drop.id,d&&d.name,"",true);return'<article '+(drop.collectible?'':itemDetailAttributes(drop.id))+' class="loot-drop '+(drop.collectible?'collection-drop ':'')+(done?drop.status:"")+'">'+art+'<span class="loot-drop-copy"><strong>'+esc((d?d.name:drop.id)+(drop.qty>1?" ×"+drop.qty:""))+'</strong><small>'+esc(d?d.desc||d.story:"Objeto recuperado")+'</small><em>'+esc(status)+'</em></span><div class="loot-buttons '+(dis?'has-disassembly':'')+'"><button data-take-loot="'+i+'" '+(done?'disabled':'')+'>'+(drop.status==="taken"?"Tomado":"Tomar")+'</button>'+dis+'<button class="discard" data-discard-loot="'+i+'" '+(done?'disabled':'')+'>'+(drop.status==="discarded"?"Descartado":"Descartar")+'</button></div></article>'}).join("");
  $("takeAllLoot").textContent="Saquear todo · "+pending+" espacios"+(collectibles?" · "+collectibles+" hallazgo":"");$("takeAllLoot").classList.toggle("hidden",pending+collectibles===0);Array.prototype.forEach.call(document.querySelectorAll("[data-take-loot]"),function(b){b.addEventListener("click",function(){takeLoot(Number(b.dataset.takeLoot))})});Array.prototype.forEach.call(document.querySelectorAll("[data-disassemble-loot]"),function(b){b.addEventListener("click",function(){openDisassemblyFromLoot(Number(b.dataset.disassembleLoot))})});Array.prototype.forEach.call(document.querySelectorAll("[data-discard-loot]"),function(b){b.addEventListener("click",function(){discardLoot(Number(b.dataset.discardLoot))})});$("lootModal").classList.remove("hidden");$("closeLoot").focus()
}
