...
function eventDisplay(ev,index){var result=legacyEventDisplay(ev,index);result=typeof narrativeEvent==="function"?narrativeEvent(result):result;result=typeof narrativeRememberedEvent==="function"?narrativeRememberedEvent(result,index):result;return typeof finaleEvent==="function"?finaleEvent(result,index):result}
...
function render(){
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-choice]"),function(b){b.addEventListener("click",function(){choose(Number(b.dataset.choice))})});renderSignalHud();renderMini();if(typeof fieldScheduleOffer==="function")fieldScheduleOffer()
}
function renderMini(){
...
function openPanel(type){
  ...
  if(type!=="missions"&&type!=="archive"&&type!=="decisions")html=list.length?list.map(function(x){return'<article class="drawer-item"><strong>'+esc(x[0])+"</strong><p>"+esc(x[1])+"</p></article>"}).join(""):'<p class="empty">Todavía no hay registros en esta sección.</p>';
