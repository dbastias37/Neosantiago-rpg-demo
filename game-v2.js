...
function renderProfile(i,feedback){
  ...
  var bag=p.bag.length?p.bag.map(function(entry,index){var d=gear(entry.id),equip=canEquip(p,d),usable=profileItemUsable(p,entry.id),label=d&&d.kind==="weapon"&&p.categories.indexOf(d.category)<0?"Categoría incompatible":"Equipar",targets=state.party.map(function(other,j){return j!==i&&canReceive(other,entry.id,1)?'<option value="'+j+'">'+esc(other.name)+' · '+bagUsed(other)+'/'+bagCapacity(other)+'</option>':""}).join(""),units=Math.max(1,Number(entry.qty)||1),transferMax=d&&d.stack?units:1,dur=d&&d.maxDurability?' · Durabilidad '+entry.durability+'/'+d.maxDurability:"",actions=(d&&d.slot?'<button class="equip-btn" data-equip="'+index+'" '+(!equip?'disabled':'')+'>'+esc(label)+'</button>':"")+(usable||["food","meds","bandage","medkit"].indexOf(entry.id)>=0?'<button class="profile-use-btn" data-profile-use="'+index+'" '+(!usable?'disabled':'')+'>Usar</button>':"");return'<article class="bag-item">'+itemArt(entry.id,d&&d.name)+'<span class="bag-copy"><b>'+esc((d?d.name:entry.id)+(entry.qty>1?" ×"+entry.qty:""))+'</b><small>'+esc((d?d.desc:"Objeto recuperado")+dur)+'</small></span><div class="bag-actions">'+actions+'<input class="transfer-qty" data-transfer-qty="'+index+'" type="number" min="1" max="'+transferMax+'" value="1" '+(!targets?'disabled':'')+' aria-label="Cantidad a transferir de '+esc(d?d.name:entry.id)+'"><select class="transfer-select" data-transfer-to="'+index+'" '+(!targets?'disabled':'')+' aria-label="Destinatario de '+esc(d?d.name:entry.id)+'"><option value="">Transferir a…</option>'+targets+'</select><button class="transfer-confirm" data-transfer-confirm="'+index+'" '+(!targets?'disabled':'')+'>Pasar</button></div></article>'}).join(""):'<p class="empty">La mochila está vacía.</p>';
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-transfer-confirm]"),function(b){b.addEventListener("click",function(){var index=Number(b.dataset.transferConfirm),target=document.querySelector('[data-transfer-to="'+index+'"]'),qty=document.querySelector('[data-transfer-qty="'+index+'"]');if(target&&target.value!==""){playSfx("loadout-transfer");transferBagItem(i,index,Number(target.value),qty?Number(qty.value):1)}else toast("Elige a quién pasarle el objeto")})});
