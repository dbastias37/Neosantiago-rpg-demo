...
function narrativeVisit(id,index){
  ...
  var log=narrativeRouteLog(id),scene=def.scenes[index];if(log.visited.indexOf(scene.id)<0)log.visited.push(scene.id);
}
...
function narrativeGraph(id){
  ...
  def.scenes.forEach(function(s){
    ...
    s.options.forEach(function(o){var chosen=log.choices.indexOf(o.id)>=0,target=o.end?o.id+".end":def.scenes[o.nextScene].id;
      if(o.end)nodes.push({id:target,column:3,label:chosen?o.label:"Desenlace no descubierto",seen:chosen,status:chosen?"Vivido":"Sin descubrir"});
      edges.push({from:s.id,to:target,label:seen?o.label:"",chosen:chosen,combat:!!o.combat});
    });
  });
  return{title:def.title,nodes:nodes,edges:edges,legacy:!state.narrative||!state.narrative.routes[id],completed:!!log.completed};
}
function drawNarrativeMap(canvas,id){
  var graph=narrativeGraph(id),ctx=canvas.getContext("2d"),width=2400,height=1500;
  ...
  ctx.font="28px sans-serif";ctx.fillText(graph.title+" · "+(graph.completed?"Ruta completada":"Recorrido registrado"),70,131);
  ...
  ctx.font="20px sans-serif";ctx.fillStyle="#a9bcc6";ctx.fillText("Etapa 1 · Ruta piloto de Lira. Las conexiones representan la ruta; elegir un combate no garantiza alcanzar la escena siguiente.",70,1430);
...
function downloadDecisionMap(){
  try{var canvas=document.createElement("canvas");drawNarrativeMap(canvas,"liraExiledCore");canvas.toBlob(function(blob){
    if(!blob){toast("No se pudo preparar la imagen. Inténtalo de nuevo.");return}
    var url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="NeoSantiago-mapa-de-decisiones-Lira.png";document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url)},30000);
  },"image/png")}catch(e){toast("No se pudo descargar el mapa en este navegador.")}
}
function narrativeMapPanel(){
  var graph=narrativeGraph("liraExiledCore");
  return '<article class="drawer-item"><strong>Mapa de decisiones · Lira</strong><p>Imagen horizontal de tu recorrido. Las escenas no descubiertas permanecen ocultas. Esta primera etapa cubre el desvío de Lira.</p><button id="downloadDecisionMap">Descargar mapa de decisiones PNG</button></article><div style="overflow-x:auto"><canvas id="decisionMapPreview" role="img" aria-label="Mapa horizontal del desvío de Lira. El recorrido también se describe debajo." style="width:960px;max-width:none;height:600px"></canvas></div>'+graph.nodes.filter(function(n){return n.seen}).map(function(n){return '<article class="drawer-item"><strong>'+esc(n.label)+'</strong><p>'+esc(graph.edges.filter(function(e){return e.from===n.id&&e.chosen}).map(function(e){return e.label+(e.combat?" (intento con combate)":"")}).join("; ")||n.status)+'</p></article>'}).join("");
}

function narrativeDialogue(dialogue){
  if(dialogue!==branchDialogueDefs.liraWounded)return dialogue;
  var f=state.flags;if(!f.liraBrotherRecovered&&!f.liraBrotherLost)return dialogue;
