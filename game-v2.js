...
function registerExposedMovement(){
  if(state.finished||state.refuge.active||state.inhibitor.active)return false;state.inhibitor.exposed=true;state.inhibitor.exposedMoves=(state.inhibitor.exposedMoves||0)+1;save();if(state.inhibitor.exposedMoves>=trackingMoveLimit()){openSignalWarning("tracking");return true}renderSignalHud();return false
}
...
function startTrackingCombat(source){
  var day=currentDay(),encounters=day===1?[["drone"],["drone","drone"]]:day===2?[["drone","drone"],["agent3","drone"]]:[["agent3","drone"],["agent2","drone","drone"]];state.inhibitor.active=false;state.inhibitor.exposed=true;state.inhibitor.needsSync=true;state.inhibitor.exposedMoves=0;if(source!=="hack"){state.inhibitor.alarms++;state.stats.signalAlarms++;state.threat=clamp(state.threat+3,0,100)}state.stats.trackingCombats++;save();startCombat({title:"Rastreo de la Red UNO",brief:"Las señales vitales del grupo quedaron expuestas. Neutraliza la patrulla, huye al refugio o utiliza equipo para sobrevivir.",encounters:encounters},{title:"Contacto por rastreo",result:"El grupo rompe el cerco de vigilancia, pero su firma continúa visible.",_signalTracking:true,psy:{stress:1,resolve:1}})
}
...
function newGame(){state=fresh();pending=null;battleState=null;transferDraft=null;discardDraft=null;disassemblyState=null;npcDialogueState=null;routeNarrativeState=null;signalGameState=null;archiveOpener=null;audioClueOpener=null;worldLoreOpener=null;signalLastTick=Date.now();signalSaveAccumulator=0;clearInterval(lootInterval);clearInterval(disassemblyTimer);clearInterval(npcDialogueTimer);resetSignalRoundTimer();clearRouteNarrativeTimers();resetAudioCluePlayer();["titleScreen","start","gameIntro","worldLoreModal","battle","final","summary","refuge","profileModal","transferModal","discardModal","disassemblyModal","lootModal","logisticsModal","archiveModal","audioClueModal","npcDialogueModal","routeNarrativeModal","signalModal","signalWarningModal"].forEach(function(id){$(id).classList.add("hidden")});render();save();showGameIntro(0)}
function continueGame(){if(!load()){newGame();return}signalLastTick=Date.now();$("titleScreen").classList.add("hidden");$("start").classList.add("hidden");render();if(!state.introCompleted)showGameIntro(0);else if(state.finished&&state.ending){if(state.summarySeen)showRunSummary();else finish(state.ending)}else if(state.refuge.active)openRefuge(state.refuge.reason,true);else{toast("Partida restaurada");if(state.inhibitor.needsSync&&!state.inhibitor.active)setTimeout(function(){openSignalHack("restore")},320)}}
