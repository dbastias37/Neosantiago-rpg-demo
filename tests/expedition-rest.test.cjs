...
test('sharing consumes one food and water for the group, shows capped recovery, persists a receipt and pays once',()=>{
 ...
 const b=boot(a.storage);b.ctx.continueGame();assert.equal(b.ctx.pendingNight().phase,'settled');assert.equal(snapshot(b.ctx),after);assert.equal(b.ctx.settleNight('save-food'),false);assert.equal(b.ctx.continueAfterNight(),true);assert.equal(b.ctx.continueAfterNight(),false);assert.equal(b.ctx.pendingNight(),null);const prepared=JSON.parse(after);prepared.stats.refugeVisits++;assert.equal(snapshot(b.ctx),JSON.stringify(prepared));assert.equal(b.ctx.state.refuge.reason,"preparation");
});
