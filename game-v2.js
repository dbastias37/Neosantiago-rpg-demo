...
function openRefugeHelp(topic,opener){
  ...
  if(topic==="economy"){
    ...
    $("refugeHelpText").textContent=(state.starterKitGiven?"Ya recibiste el lote inicial de Mara. Las fichas son el dinero compartido del grupo.":"Primero pulsa Recibir lote inicial: Mara entrega gratis 10 unidades de suministros y 6 fichas. Las fichas son el dinero compartido del grupo.")+"\n\nVender: en Tu loot, pulsa Entregar. Sale una unidad de la mochila y recibes las fichas indicadas.\n\nComprar: en Reservas de Mara, pulsa Recibir. Pagas el precio indicado y el objeto se guarda en una mochila con espacio. Cada compra o venta mueve una unidad.\n\nEl stock es limitado y se renueva cada día. El Armero usa las mismas fichas para vender armas; equípalas desde la ficha del aliado.";
    $("refugeHelpTip").textContent="Guarda comida, medicina y materiales que puedas necesitar para curar, fabricar o reparar. Puedes volver a leer esta ayuda desde Fichas · ayuda.";
  }else{
    $("refugeHelpTitle").textContent="Para qué sirve descansar";
    $("refugeHelpText").textContent="Descansar y estabilizar recupera al grupo antes de volver a la superficie.\n\nCada aliado recupera 8 HP. Si está agotado, con 0 HP, vuelve a 12 HP. Todos recuperan 25 puntos de energía y baja su tensión, sin superar sus valores máximos.\n\nEs gratis: no gasta fichas ni comida. Puedes hacerlo una vez por visita al refugio.\n\nSi necesitas recuperar la moral del grupo, utiliza Reagrupar al equipo.";
...
var eventAudioClues={
  ...
  13:[{id:"terminal-memory",title:"Fragmento del terminal",kicker:"Memoria dañada",source:"Nodo 14 · Terminal de rutas",duration:"00:20",src:"audio/lore/voice/terminal-memory.mp3",summary:"La máquina confirma que la lista de refugios puede salvar comunidades o convertirlas en objetivos.",transcript:"Consulta incompleta. Refugios no registrados: ciento doce. Identidades detectadas: activas. Origen de intervención: soporte vital humano. Riesgo de transmisión: alto."}],
...
function choose(i){
  ...
  if(!c._noaRecorded)drainHunger(4);
...
function completeChoice(choice,out,roll,check,extra){
  ...
  if(!choice._noaRecorded&&typeof prepareCrate==="function")prepareCrate(choice,out);
...
function selectNpcDialogueChoice(i){
  if(!npcDialogueState||npcDialogueState.selected)return;if(npcDialogueTimer){revealNpcDialogueText();return}var opt=npcDialogueState.options[i];if(!opt||reason(opt))return;npcDialogueState.selected=true;if(opt.combat){closeNpcDialogueForCombat(opt);return}
...
function eventDisplay(ev,index){var result=legacyEventDisplay(ev,index);result=typeof narrativeEvent==="function"?narrativeEvent(result):result;result=typeof narrativeRememberedEvent==="function"?narrativeRememberedEvent(result,index):result;result=typeof companionCareEvent==="function"?companionCareEvent(result,index):result;result=typeof companionRouteEvent==="function"?companionRouteEvent(result,index):result;return typeof finaleEvent==="function"?finaleEvent(result,index):result}
...
var gameHelpPages=[
  ...
  {title:"Recuperar y preparar",text:"Después de vencer, elige quién recoge el loot. Cada unidad necesita espacio en su mochila: reparte objetos entre los aliados para poder llevar lo que te sirva.\n\nDesde sus fichas puedes equipar, transferir y fabricar. Sara prepara medicina, Elías trabaja con piezas y Noa fabrica granadas y EMP. Las recetas indican qué materiales consumen.",tip:"Mara compra tu loot a cambio de fichas y vende suministros. El Armero ofrece armas; equípalas desde la ficha del aliado."},
