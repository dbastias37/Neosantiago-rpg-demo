...
function statusCard(p,label,enemy,looting){
 ...
 var synergy=!enemy&&!looting&&typeof renderBattleTactics==='function'?'<div class="stage-meter synergy-status"><span>Sinergia</span><div class="bar synergy-meter" role="progressbar" aria-label="Sinergia del equipo" aria-orientation="horizontal" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'+fill+'"><span id="synergyFill" class="'+(fill!==previous?'growing':'')+'" style="width:'+fill+'%;--synergy-from:'+previous+'%;--synergy-to:'+fill+'%"></span></div><b id="synergyValue">'+fill+'%</b></div>':'';
 return '<section class="stage-status-card '+(enemy?'hostile':'')+'"><div class="status-heading"><strong>'+esc(p.name)+'</strong><span>'+esc(label)+(enemy?'':' · Lvl. '+p.level)+'</span></div>'+(looting&&!enemy?cargo:'<div class="stage-meter"><span>HP</span><div class="bar"><span style="width:'+(100*hp/p.maxHp)+'%"></span></div><b>'+hp+'/'+p.maxHp+'</b></div>'+xp+synergy)+'</section>';
}
