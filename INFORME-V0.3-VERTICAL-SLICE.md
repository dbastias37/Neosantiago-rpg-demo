...
## Interfaz del inhibidor — 24 septiembre 2026

Base: `main` después de integrar la continuidad de escenas. El dispositivo de Elías ahora presenta el código completo sobre una pantalla cartográfica de señal, con la posición actual, y cuatro teclas físicas en disposición de flechas de PC. La secuencia usa una cuadrícula de tantas columnas como pasos, sin saltos de línea; su caso máximo actual es de once flechas en el tercer día y la tercera ronda. En horizontal corto, pantalla y teclado se colocan juntos para conservar los controles a la vista. Las teclas táctiles conservan el tamaño mínimo de 44 px y las entradas de teclado ya existentes (flechas y W A S D) permanecen activas.

Es una extracción de estilos del panel existente a `styles/inhibitor-device.css` y un cambio de presentación en `renderSignalRound`; se retira la segunda hilera que repetía las entradas. No cambia secuencias, temporizadores, errores, resultados, guardados, rutas ni recompensas. El plano muestra un circuito abstracto de pulsos: no atribuye una ubicación nueva a la Red UNO. La captura E2E `inhibidor-mapa-y-teclado.png` se produce para cada resolución en los artefactos de Playwright.

Verificación: **467/467 pruebas Node y 84/84 E2E**, además de sintaxis, referencias y catálogo de audio aprobados. Se regeneraron los índices de línea de la auditoría de audio tras modificar el HTML; permanecen 37 archivos y 64 referencias ausentes ya conocidas, sin deuda nueva. El caso dirigido de Playwright aprueba en las siete resoluciones, comprueba que once flechas caben en una sola fila sin desbordamiento, que las teclas siguen visibles y son accionables con ratón/tacto y teclado. La matriz E2E completa recorre campaña y Encargos sin regresiones detectadas. Pendientes específicos: revisión en dispositivos móviles reales y evaluación del combate por separado.
