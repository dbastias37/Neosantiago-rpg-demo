...
function showRouteNarrativeOptions(){
  ...
  routeNarrativeState.optionsVisible=true;renderRouteNarrativeChoices();$("routeNarrativeChoices").classList.remove("hidden");$("routeNarrativeClock").classList.remove("hidden");$("routeNarrativeButton").textContent="Elegir ruta";$("routeNarrativeButton").disabled=true;clearInterval(routeNarrativeTimer);routeNarrativeTimer=setInterval(function(){var elapsed=Date.now()-start,pct=clamp(100-Math.round(100*elapsed/duration),0,100);$("routeNarrativeTimeBar").style.width=pct+"%";if(elapsed>=duration){clearInterval(routeNarrativeTimer);routeNarrativeTimer=null;selectRouteNarrativeChoice(routeDefaultChoice(scene,def))}},100);var first=$("routeNarrativeChoices").querySelector("button:not(:disabled)");if(first)first.focus({preventScroll:true})
}
...
function render(){
  ...
  $("choices").innerHTML=ev.choices.map(function(c,i){var why=reason(c);return'<button class="choice'+(c.roll?' choice-check':'')+'" data-choice="'+i+'" '+(why?"disabled":"")+'><span class="num">'+(i+1)+'</span><span><strong>'+esc(c.label)+"</strong><small>"+esc(choiceHintText(c,why))+"</small></span><span class=\"cost\">"+esc(decisionCostText(c))+"</span></button>"}).join("");
