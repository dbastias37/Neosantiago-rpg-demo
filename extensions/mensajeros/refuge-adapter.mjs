...
function card(id,qty,member,selling){
 ...
 return `<article class="trade-card ${i.kind==='weapon'?'weapon-offer':''}">${art(id)}<span><b>${esc(i.name)}${qty>1?' ×'+qty:''}</b><small>${selling?esc(name(member))+' · entrega 1 unidad':esc(i.description)}</small></span><em>${selling?'+':'−'}${price} créditos</em><button data-${selling?'sell':'buy'}="${id}" data-member="${member}" ${!selling&&w.credits<price?'disabled':''}>${selling?'Entregar':vendor==='armorer'?'Encargar':'Recibir'}</button></article>`;
}
function render(){
 ...
 $('tradeCredits').textContent=w.credits+' créditos';$('tradeSectionTitle').textContent=arm?'Armas reconstruidas':'Compra y venta';
 ...
 $('refugeRest').textContent='Recuperar grupo · 8 créditos';$('refugeRest').disabled=w.credits<8||party().every(c=>c.hp===c.maxHp);
