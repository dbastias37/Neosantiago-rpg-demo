...
function boot(storage = new Map(), options = {}) {
  ...
  let doc={getElementById(id){if(!nodes.has(id))throw Error('Missing DOM id '+id);return nodes.get(id)},querySelectorAll(){return []},querySelector(){return element()},createElement:()=>element(),body:element(),documentElement:element(),hidden:false,addEventListener(type,fn){(listeners[type] ||= []).push(fn)}};
  if(options.document){doc=options.document;nodes.clear();for(const node of doc.querySelectorAll('[id]'))nodes.set(node.id,node)}
  const ctx={innerHeight:844,console,document:doc,localStorage:{getItem:k=>storage.get(k)||null,setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)},setTimeout(fn,ms){timers.set(++timerId,{fn,ms});return timerId},setInterval(fn,ms){timers.set(++timerId,{fn,ms,interval:true});return timerId},clearTimeout:id=>timers.delete(id),clearInterval:id=>timers.delete(id),getComputedStyle(){return {backgroundImage:'none'}},requestAnimationFrame(){},Audio:function(){return element()},Image:function(){return element()},navigator:{},location:{search:options.search||'',hash:''},matchMedia:()=>({matches:false,addEventListener(){}}),addEventListener(){},performance:{now:()=>0},URL,Blob};
