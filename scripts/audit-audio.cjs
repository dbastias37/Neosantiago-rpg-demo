...
function walk(folder, accept, skip = new Set()) {
  return fs.readdirSync(path.join(root, folder), {withFileTypes: true}).sort((a,b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0).flatMap(entry => {
    if (skip.has(entry.name) || /^test-results(?:-|$)/.test(entry.name) || entry.name.startsWith('.playwright-artifacts')) return [];
...
function artifactContents(report) {
  const catalog=catalogFor(report);
  ...
  return {
    'audio-catalog.js':output,
    'docs/v0.3/audio-audit.json':JSON.stringify(report,null,2)+'\n',
    'docs/v0.3/audio-audit.md':markdown(report)
  };
}
function staleArtifacts(report,readFile=file=>fs.existsSync(path.join(root,file))?fs.readFileSync(path.join(root,file),'utf8'):null) {
  return Object.entries(artifactContents(report)).filter(([file,content])=>readFile(file)!==content).map(([file])=>file);
}
function main() {
  const report=audit();
  if(process.argv.includes('--check')) {
    const stale=staleArtifacts(report);
    if(stale.length) {console.error('Audio artifacts are stale: '+stale.join(', ')+'. Run node scripts/audit-audio.cjs.');process.exitCode=1;return;}
  } else {
    fs.mkdirSync(path.join(root,'docs/v0.3'),{recursive:true});
    for(const [file,content] of Object.entries(artifactContents(report)))fs.writeFileSync(path.join(root,file),content);
  }
  console.log(JSON.stringify(report.summary));
}
if(require.main===module)main();
module.exports={audit,catalogFor,artifactContents,staleArtifacts};
